...
const NonMemoizedMarkdown = ({ children }: { children: string }) => {
  const components = {
    code: ({ node, inline, className, children, ...props }: any) => {
      ...
      return !inline && match ? (
        <div className="my-6">
          <div className="flex justify-between bg-base-300 text-foreground px-4 py-1.5 rounded-t-lg border border-white/10 border-b-0 text-sm bg-accent">
            {language}
          ...
          <SyntaxHighlighter
            ...
            language={language}
            style={dark}
            className="!bg-accent shadow-md border border-white/10 rounded-b-lg px-4 py-2 leading-6 !m-0"
          >
            {children}
          </SyntaxHighlighter>
        </div>
      ) : (
        <code className={`${className} rounded-md px-1.5 py-0.5 bg-accent`} {...props}>
          {children}
        </code>
      );
    },
    ...
    table: ({ node, children, ...props }: any) => (
      <table className="w-full border border-white/10 my-6 rounded-md overflow-hidden" {...props}>
        {children}
      </table>
    ),
    th: ({ node, children, ...props }: any) => (
      <th className="border border-white/10 px-3 py-2 text-center text-sm font-medium bg-base-300/30" {...props}>
        {children}
      </th>
    ),
    td: ({ node, children, ...props }: any) => (
      <td className="border border-white/10 px-3 py-2 text-center text-sm" {...props}>
        {children}
      </td>
    ),
    ...
    a: ({ node, children, ...props }: any) => (
      <a className="text-blue-500 hover:underline font-medium" target="_blank" rel="noreferrer" {...props}>
        {children}
      </a>
    ),
